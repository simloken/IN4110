...
def extract_events(table: bs4.element.Tag) -> pd.DataFrame:
    ...
    for i in temp:
        ...
        for j in i:
            k += 1
            if any(x in j.split(' ') for x in ['2020', '2021', '2022', '2023']) and len(j) < 30:
                if '[' in j: #removes references
                    t = re.sub(r'\[[^)]*\]', '', j) #regex finds and removes anything within brackets
                    data.append(t)
                else:
                    data.append(j)
                if i[k + 1][0] == ' ': #all venues have blank space first (because of the flag I presume?)
                    data.append(i[k+1])
                    prev_venue = i[k+1]
                else:
                    data.append(prev_venue)
            try:
                event_types[j.split(' ')[0]]
            except:
                continue
            else:
                data.append(event_types[j.split(' ')[0]])
                

    # List of desired columns
    
    df = pd.DataFrame([data[i:i+3] for i in range(0, len(data), 3)], columns=['Date', 'Venue', 'Type'])
